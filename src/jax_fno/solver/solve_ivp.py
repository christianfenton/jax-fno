...
from .timesteppers.stepper import Stepper
from .timesteppers.implicit import BackwardEuler, default_linear_solver
...
# Helper type for a function df : u, t -> g where g : v -> (∂f/∂u)*v
#   In other words, df returns a matrix-free implementation of ∂f/∂u evaluated at u, t
dfType = Callable[[jnp.ndarray, float], Callable[[jnp.ndarray], jnp.ndarray]]
...
def _integrate_implicit(
    ...
    Kwargs:
        tol: Convergence tolerance in Newton-Raphson iterations. Default: 1e-6.
        ...
        linsolver: Iterative solver used on the linear systems inside the Newton-Raphson iterations.
            Default: GMRES ('jax.scipy.sparse.linalg.gmres') with tol=1e-6 and maxiter=100.
    """
    ...
    def body_fn(i, carry):
        u, t = carry
        u_next = stepper.step(f, u, t, dt, df=df, linsolver=linsolver, tol=tol, maxiter=maxiter)
        t_next = t + dt
        return (u_next, t_next)
...
def integrate(
    ...
    **stepper_kwargs
    ...
    Args:
        ...
        dt: Time step size
        stepper: Time-stepping method object (e.g., RK4(), ForwardEuler(), BackwardEuler())
    ...
    Returns:
        ...
        t_final: Final time
    """
    if stepper.method_type == 'explicit':
        return _integrate_explicit(f, u0, t_span, dt, stepper)
    elif stepper.method_type == 'implicit':
        return _integrate_implicit(f, u0, t_span, dt, stepper, **stepper_kwargs)
    else:
        raise ValueError(f"Unknown method_type: {stepper.method_type}")
...
def solve_ivp(
    ...
    **stepper_kwargs
    ...
    Args:
        ...
        save_every: Save every N steps. If None, saves only initial and final states.
    ...
    if verbose:
        ...
        print(f"Time: [{t_start}, {t_end}], dt={dt}, {n_steps_total} total steps")
    ...
    if verbose:
        print(f"Completed in {elapsed_time:.3f}s ({n_steps_total/elapsed_time:.1f} steps/s)")
